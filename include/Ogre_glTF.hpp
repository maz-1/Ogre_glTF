...
namespace Ogre_glTF
{
	...
	public:
		...
		///Release resources created by this adapter. Call only after destroying every Ogre object
		///and stopping use of every datablock returned by the legacy API.
		void releaseResources();
	};

	///A scene instance that owns the Ogre objects it creates and keeps their asset alive.
	///Destroy or reset it before its SceneManager and Ogre::Root are shut down.
	class Ogre_glTF_EXPORT SceneInstance
	{
		friend class loaderAdapter;
		friend class ManagedAsset;
		std::shared_ptr<loaderAdapter> adapter;
		Ogre::SceneManager* sceneManager = nullptr;
		std::vector<Ogre::SceneNode*> roots;
	...
	public:
		...
		SceneInstance(SceneInstance&& other) noexcept;
		SceneInstance& operator=(SceneInstance&& other);
		void reset();
		Ogre::SceneNode* getRootNode() const { return roots.empty() ? nullptr : roots.front(); }
		const std::vector<Ogre::SceneNode*>& getRootNodes() const { return roots; }
	};

	///A managed glTF asset. Its Ogre resources remain alive while the asset or any scene instance lives.
	class Ogre_glTF_EXPORT ManagedAsset
	{
		friend class glTFLoader;
		std::shared_ptr<loaderAdapter> adapter;
		explicit ManagedAsset(std::shared_ptr<loaderAdapter> input) : adapter(std::move(input)) {}

	public:
		ManagedAsset() = default;
		SceneInstance instantiate(Ogre::SceneNode* parentNode, Ogre::SceneManager* smgr) const;
		bool isOk() const { return adapter && adapter->isOk(); }
	};
	...
	public:
		...
		ManagedAsset loadManagedFromFileSystem(const std::string& path) const;

		///Load a GLB from Ogre's resource manager
		loaderAdapter loadGlbResource(const std::string& name) const;
		ManagedAsset loadManagedGlbResource(const std::string& name) const;
